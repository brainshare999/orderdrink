...
import { Mail, Lock, X, LogIn, UserPlus, Sparkles, AlertCircle, CheckCircle2, Loader2 } from 'lucide-react';

export const AuthModal: React.FC = () => {
  const { isAuthModalOpen, setIsAuthModalOpen, authMode, setAuthMode, signInWithEmail, signUpWithEmail } = useAuth();
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-white">
                {authMode === 'login' ? '會員登入' : '註冊新會員帳號'}
              </h3>
              <p className="text-xs text-amber-200/80">拾茶時光・專屬雲端訂單同步服務</p>
            </div>
          </div>
          <button
            onClick={() => !loading && setIsAuthModalOpen(false)}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {/* Error Message */}
          {errorMsg && (
            <div className="p-3 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 rounded-2xl flex items-start gap-2.5 text-xs text-rose-800 dark:text-rose-300 animate-shake">
          ...
          {/* Success Message */}
          {successMsg && (
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/60 rounded-2xl flex items-start gap-2.5 text-xs text-emerald-800 dark:text-emerald-300 animate-fade-in">
